...
function populate(match_history, specificRow, discludeLeague) {
  ...
  for(n = 0; n < match_history.length; n++) {
    ...
    setCell('WIP ROW', row, true);
    setCell('Patch', row, getPatch(match));
    var dt = getMatchDate(match);
    setCell('Date', row, dt[0]);
    setCell('Time', row, dt[1]);
    var matchLength = getMatchLength(match);
    setCell('Length', row, matchLength);
    if(matchLength < 6){
      setCell('Result', row, 'Remake');
      setCell('WIP ROW', row, '');
      continue;
    }
    ...
    setCell('Kill Contribution', row, (stats['kills'] + stats['assists'])/getTotalKD(match, teamId, 'kills'));
    ...
    setCell('Damage Participation', row, damageToChamps);
    ...
    getDeltas(pobj, row);
    var oppPobj = getOpponentParticipantObj(match, getMyRole(row), teamId, row);
    ...
    setCell('Damage Participation Diff', row, damageToChamps-laneOpponentStats['damageToChamps']);
...
function getMatchHistoryIds(mode) {
  ...
  var url = 'https://' + getInfo('region') + '.api.riotgames.com/lol/match/v3/matchlists/by-account/'+ getInfo('account_id')+ '?beginTime=1493564400000&queue=420&api_key=' + getInfo('api_key'); 
  ...
  if(!status) {
    ...
    for(i = 0; i < data["matches"].length; i++) {
      if(data["matches"][i]["gameId"] != "undefined") {
...
function getMatch(matchId) {
  var url = 'https://' + getInfo('region') + '.api.riotgames.com/lol/match/v3/matches/'+ matchId + '?api_key=' + getInfo('api_key');
  var response = UrlFetchApp.fetch(url, {muteHttpExceptions: true});
  var status = checkStatusError(response);
  if(!status) {
    var json = response.getContentText();
...
function getMatchDate(match) {
  ...
  var day = Utilities.formatDate(d, 'JST', "yyyy-MM-dd");
  var time = Utilities.formatDate(d, 'JST', "HH:mm");
...
function getOpponentParticipantObj(match, role, teamId, row) {
  ...
  var s = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = s.getSheetByName('Data');
  var opponentChampion = sheet.getRange(row, getSheetTranslationIndex('Their ' + role)).getValue();

  var participants = match['participants'];
  for(i = 0; i < participants.length; i++) {
    if(participants[i]['teamId'] != teamId) {
      if(getChampionTranslation(participants[i]['championId']) == opponentChampion) {
        return participants[i];
      }
    }
  }
...
function getDeltas(participant, row) { 
  ...
  var deltaTypes = ['creepsPerMinDeltas', 'goldPerMinDeltas', 'csDiffPerMinDeltas', 'xpPerMinDeltas', 'xpDiffPerMinDeltas'];
  var deltaColumns = ['CS/Min Delta', 'Gold Delta', 'CS/Min Diff Delta', 'XPM Delta', 'XPM Diff Delta'];
  ...
  for(var deltaCount = 0; deltaCount <= 4; deltaCount++) {
